...
module.exports = {
    getClient(client, id) {
        return db.selectOne(client, "select oc.* from oauth2_clients oc where oc.id = ?", [id]);
    },

    getClients(client, id) {
        return db.selectAll(client, "select oc.* from oauth2_clients oc where oc.id = ?", [id]);
    },
    ...
    getAllPermissionsOfUser(dbClient, userId) {
        return db.selectAll(dbClient, `select oc.* from oauth2_clients oc,
            oauth2_permissions op where oc.id = op.client_id and op.user_id = ?`, [userId]);
    }
};
